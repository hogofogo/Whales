...
# -*- coding: utf-8 -*-
"""
Created on Sun Feb 11 09:24:03 2018
...
#THIS FILE CONTAINS FUNCTIONS NEEDED TO CREATE MODEL, FIT MODEL, DEFINE TRIPLET LOSS,
#GENERATE ANCHOR, POSITIVE AND NEGATIVE ARRAYS FROM IMAGE FILES, AND RUN THE TRAINING

np.random.seed(88)

def WhaleModel(input_shape):
    """
    ...
    ###!!! there is no activation in this module and output is a fully connected layer
    X = Dense(121, name='predictions')(X)
    
    # L2 normalization
    X = Lambda(lambda  x: K.l2_normalize(x,axis=1))(X)

    # Create model
    model = Model(inputs = X_input, outputs = X, name='WhaleModel')
...
out_a = whaleModel(in_a)
out_p = whaleModel(in_p)
out_n = whaleModel(in_n)
...
def generate_arrays_from_files_BATCH(start = 0, batch_size = 320):
